function renderTalks() {
    ...
    const t = e => {
        ...
        function c(n) {
            o(e) != y && (n.target.removeEventListener(n.type, arguments.callee),
            t(e))
        }
        ...
        for (var m = 1; m < d.length; m++) {
            ...
            if (!(l(u) + o(h) <= o(e)))
                break;
            h.style.top = u.style.top,
            h.style.left = a(l(u) + n("Left", h)),
            p.push(h)
        }
        for (; m < d.length; m++) {
            r(p);
      ...
      , n = n => {
        n.map(a).forEach((t => e.appendChild(i(t)))),
        t("#talk")
    }
      , a = e => {
        const t = s(e.created_at);
        let n = e.content || "";
        if (n = n.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" rel="nofollow noopener">@$1</a>').replace(/- \[ \]/g, "⚪").replace(/- \[x\]/g, "⚫").replace(/\n/g, "<br>"),
        n = `<div class="talk_content_text">${n}</div>`,
        Array.isArray(e.images) && e.images.length > 0) {
            const t = document.createElement("div");
            t.className = "zone_imgbox",
            e.images.forEach((e => {
                const n = document.createElement("a");
                // 检查图片URL是否为完整URL，如果不是则添加前缀
                let imageUrl = e.image_url;
                if (!imageUrl.startsWith('http://') && !imageUrl.startsWith('https://')) {
                    imageUrl = 'https://echo.sunboy.ltd/api' + imageUrl;
                }
                // 移除可能存在的查询参数
        ...
        if (["WEBSITE", "GITHUBPROJ"].includes(e.extension_type)) {
            let t = ""
              , a = ""
              , i = "https://image.sunboy.ltd/20251110232026415.avif";
            try {
                const n = "string" == typeof e.extension ? JSON.parse(e.extension) : e.extension;
                t = n.site || n.url || e.extension,
                a = n.title || t
            } catch {
                t = e.extension,
                a = t
            }
            if ("GITHUBPROJ" === e.extension_type) {
                i = "https://image.sunboy.ltd/20251110232039013.avif";
                const e = t.match(/^https?:\/\/github\.com\/[^/]+\/([^/?#]+)/i);
                if (e)
                    a = e[1];
                else
                    try {
                        a = new URL(t).pathname.split("/").filter(Boolean).pop() || t
                    } catch {}
            }
            n += `\n                <div class="shuoshuo-external-link">\n                    <a class="external-link" href="${t}" target="_blank" rel="nofollow noopener">\n                        <div class="external-link-left" style="background-image:url(${i})"></div>\n                        <div class="external-link-right">\n                            <div class="external-link-title">${a}</div>\n                            <div>点击跳转<i class="fa-solid fa-angle-right"></i></div>\n                        </div>\n                    </a>\n                </div>`
        }
        if ("MUSIC" === e.extension_type && e.extension) {
            const t = e.extension;
            let a = "";
            t.includes("music.163.com") ? a = "netease" : t.includes("y.qq.com") && (a = "tencent");
            const i = t.match(/id=(\d+)/)
              , o = i ? i[1] : "";
            a && o && (n += `
                <div style="position: relative; padding: 12% 45%; margin: 10px 0 5px;">
                    <div style="position:absolute;width:100%;height:100%;left:0;top:0;">
                        <meting-js server="${a}" type="song" id="${o}" api="https://api.obdo.cc/meting?server=:server&type=:type&id=:id&auth=:auth&r=:r"></meting-js>
                    </div>
                </div>`)
        }
        if ("VIDEO" === e.extension_type && e.extension) {
            const t = e.extension;
            if (t.startsWith("BV")) {
                n += `\n                    <div style="position: relative; padding: 30% 45%; margin-top: 10px;">\n                        <iframe style="position:absolute;width:100%;height:100%;left:0;top:0;border-radius:12px;" \n                                src="${`https://www.bilibili.com/blackboard/html5mobileplayer.html?bvid=${t}&as_wide=1&high_quality=1&danmaku=0`}" \n                                frameborder="no" \n                                allowfullscreen="true" \n                                loading="lazy"></iframe>\n                    </div>`
            } else {
        ...
        return {
            ...
            date: t,
            location: "甘肃庆阳",
            tags: Array.isArray(e.tags) && e.tags.length ? e.tags.map((e => e.name)) : ["无标签"],
            text: n.replace(/\[(.*?)\]\((.*?)\)/g, "[链接]")
        }
    }
      , i = e => {
        ...
        return h.className = "icon",
        h.innerHTML = '<i class="fa-solid fa-message fa-fw"></i>',
        u.appendChild(h),
        c.appendChild(d),
        c.appendChild(u),
        t.appendChild(n),
    ...
    ( () => {
        const e = "talksCache"
          ...
          , o = Date.now();
        a && i && o - i < 18e5 ? n(JSON.parse(a)) : fetch("https://echo.sunboy.ltd/api/echo/page", {
            method: "POST",
            headers: {
            ...
            1 === a.code && a.data && Array.isArray(a.data.items) && (localStorage.setItem(e, JSON.stringify(a.data.items)),
            localStorage.setItem(t, o.toString()),
            n(a.data.items))
        }
